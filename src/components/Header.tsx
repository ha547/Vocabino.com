...
export type NavTab = "dashboard" | TestSection | "guide" | "certificate" | "fulltests" | "verify" | "blog" | "videos" | "creator";
...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur-md">
      ...
      <div className="bg-slate-900 px-4 py-1.5 text-center text-[11px] font-medium text-slate-300">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          <div className="flex items-center gap-1.5">
            <span className="flex h-2 w-2 rounded-full bg-emerald-400 animate-pulse" />
            <span className="font-extrabold text-white">Vocabino Institute of Language & Testing</span>
          ...
          <div className="flex items-center gap-3 text-[11px]">
            ...
            <button
              ...
              className={`rounded-full px-3 py-0.5 font-bold transition-colors inline-flex items-center gap-1.5 cursor-pointer ${
                currentTab === "creator"
            ...
            >
              ...
              <span>Creator & Owner: Hamid Ali</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="hidden lg:flex flex-col">
        ...
        <div className="border-t border-slate-200/80 bg-slate-50/80 py-1.5 px-3 sm:px-6">
          <div className="mx-auto max-w-7xl flex items-center justify-between">
            {/* Left side: TRF, Direct Verification, Creator & Owner, and Menu Hub button for PC */}
            <div className="flex items-center gap-2.5">
              ...
              <motion.button
                ...
                className={`flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-black transition-all border cursor-pointer ${
              ...
              <motion.button
                ...
                className={`flex items-center gap-1.5 rounded-xl border px-3 py-1.5 text-xs font-black transition shadow-2xs cursor-pointer ${
                  currentTab === "verify"
              ...
              <motion.button
                ...
                className={`flex items-center gap-1.5 rounded-xl border px-3 py-1.5 text-xs font-black transition shadow-2xs cursor-pointer ${
                  currentTab === "guide"
                    ...
                    : "border-blue-200/90 bg-blue-50/80 text-blue-900 hover:bg-blue-100 hover:border-blue-300"
                }`}
              ...
              {/* About Creator & Owner Button (Hamid Ali) */}
              <motion.button
                whileHover={{ scale: 1.03 }}
                whileTap={{ scale: 0.97 }}
                onClick={() => handleNavClick("creator")}
                className={`flex items-center gap-1.5 rounded-xl border px-3 py-1.5 text-xs font-black transition shadow-2xs cursor-pointer ${
                  currentTab === "creator"
                    ...
                    : "border-amber-300/90 bg-gradient-to-r from-amber-50 via-amber-100/60 to-amber-50 text-amber-950 hover:bg-amber-100 hover:border-amber-400"
                }`}
                title="About Creator & Platform Owner: Hamid Ali"
              >
                <UserCheck className="h-3.5 w-3.5 text-amber-700" />
                <span>Creator: Hamid Ali</span>
              </motion.button>
              ...
              <motion.button
                ...
                className="relative group rounded-full p-[1.5px] bg-gradient-to-r from-blue-700 via-indigo-600 to-slate-900 shadow-xs transition-all cursor-pointer ml-1"
      ...
      {/* MOBILE PHONE HEADER (lg:hidden) - GUARANTEED PROMINENT VOCABINO LOGO & INSTANT CREATOR ACCESS */}
      <div className="flex lg:hidden items-center justify-between px-3 py-2 border-b border-slate-100 bg-white gap-2">
        {/* LEFT ZONE: Back Button + GUARANTEED VISIBLE VOCABINO LOGO */}
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 min-w-0">
          {canGoBack && onBack && (
            ...
            >
              ...
              <span className="text-[11px] font-black">Back</span>
            </motion.button>
          )}
          ...
          <button
            ...
            title="Vocabino Institute Home"
          >
            <LingofiLogo size="sm" className="shrink-0" />
          </button>
        </div>

        {/* RIGHT ZONE: Creator Button + Creative Menu Trigger + Progress Badge */}
        <div className="flex items-center justify-end gap-1.5 shrink-0">
          {/* Creator Profile Button */}
          <motion.button
            whileTap={{ scale: 0.95 }}
            onClick={() => handleNavClick("creator")}
            className={`flex items-center gap-1 rounded-xl px-2 py-1 text-[11px] font-extrabold transition cursor-pointer shrink-0 border ${
              currentTab === "creator"
                ? "bg-amber-400 text-slate-950 border-amber-500 shadow-2xs"
                : "bg-amber-50 text-amber-950 border-amber-300 hover:bg-amber-100"
            }`}
            title="About Creator & Owner: Hamid Ali"
          >
            <UserCheck className="h-3.5 w-3.5 text-amber-700 shrink-0" />
            <span className="font-black">Creator</span>
          </motion.button>
        ...
        <motion.button
          ...
          className={`flex shrink-0 items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-black transition-all cursor-pointer ${
            currentTab === "creator"
              ...
              : "bg-gradient-to-r from-amber-50 to-amber-100 text-amber-950 border border-amber-300"
          }`}
        >
          <UserCheck className="h-3.5 w-3.5 text-amber-700" />
          About Creator
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-slate-800 bg-slate-900/90 px-4 sm:px-6 py-4">
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={() => handleNavClick("creator")}
                  className={`rounded-full border px-3 py-1 text-[11px] font-bold transition-colors inline-flex items-center gap-1.5 cursor-pointer ${
                    currentTab === "creator"
            ...
            <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-5 space-y-6 max-w-4xl mx-auto w-full">
              ...
              {/* Creator & Owner Announcement Box */}
              <div className="rounded-2xl border border-amber-400/40 bg-gradient-to-r from-amber-950/40 via-slate-900 to-slate-900 p-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2 text-xs font-black text-amber-300">
                      <Sparkles className="h-4 w-4 text-amber-400" />
                      <span>Vocabino Institute • Creator & Owner: Hamid Ali</span>
                    </div>
                  ...
                  <button
                    onClick={() => handleNavClick("creator")}
                    className="shrink-0 rounded-xl bg-amber-400 px-3.5 py-2 text-xs font-black text-slate-950 shadow-xs hover:bg-amber-300 transition-colors cursor-pointer"
                  >
                    View Creator Biography
                  </button>
                </div>
              </div>
