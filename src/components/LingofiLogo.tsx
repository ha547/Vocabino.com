...
interface VocabinoLogoProps {
  size?: "sm" | "md" | "lg" | "xl";
  variant?: "full" | "icon" | "white";
  className?: string;
}

export const VocabinoLogo: React.FC<VocabinoLogoProps> = ({
  size = "md",
  variant = "full",
  className = ""
}) => {
  ...
  const navyGradId = `vbNavyGrad_${uid}`;
  const goldLuxeId = `vbGoldLuxe_${uid}`;
  const cyanAccentId = `vbCyanAccent_${uid}`;
  const shineGlassId = `vbShineGlass_${uid}`;
  const shadowId = `vbShadow_${uid}`;
  ...
  return (
    <div className={`inline-flex items-center gap-2 sm:gap-3 select-none shrink-0 ${className}`}>
      {/* ELITE VECTOR CREST EMBLEM: STATELY VOCABINO "V" INSIGNIA */}
      <motion.div
        ...
        title="Vocabino • Official IELTS Academic Examination Institute"
      >
        <svg
          viewBox="0 0 120 120"
          fill="none"
        ...
        >
          ...
          {/* MONUMENTAL SERIF "V" FOR VOCABINO */}
          {/* Left Wing of V (Serifed & Thick) */}
          <path
            d="M32 26 H46 L60 84 L74 26 H88 L68 94 C65 100, 55 100, 52 94 Z"
            fill="#ffffff"
            filter={`url(#${shadowId})`}
          />

          {/* Inner Golden Ascending Flight Chevron */}
          <path
            d="M60 48 L76 26 H88 L68 76 Z"
            fill={`url(#${goldLuxeId})`}
            filter={`url(#${shadowId})`}
          />
...
export const LingofiLogo = VocabinoLogo;
